...
export class AddressService {
    ...
    setDefaultForCurrentUser(location: Location): Observable<HttpResponse<Location>> {
        return from(this.storage.get('userId'))
            .pipe(
                switchMap((userId: number) => {
                    const userLocation = {
                    ...
                    return this.http.post<Location>(`${this.resourceUrl}/user`, userLocation, {observe: 'response'});
                })
            );
    }
